...
(defn cents
  ...
  cents."
  [freq n-cents]
  (* freq (js/Math.pow 2 (/ n-cents 1200))))
...
(def MIDI-NOTE-RE (re-pattern (str ".*" MIDI-NOTE-RE-STR ".*")))
(def ONLY-MIDI-NOTE-RE (re-pattern MIDI-NOTE-RE-STR))
...
(defn- validate-midi-string!
  ...
  (let [matches (midi-string-matcher mk)]
    (when-not matches
      (throw (str "Invalid midi-string. " mk
...
(defn note
  ...
  (cond
    ...
    (integer? n) (if (>= n 0)
                   n
                   (throw (str "Unable to resolve note: "
                                n
                                ". Value is out of range. Lowest value is 0")))
    (keyword? n) (note (name n))
    (symbol? n) (note (name n))
    (string? n) (:midi-note (note-info n))
    :else (throw (str "Unable to resolve note: " n ". Wasn't a recognised format (either an integer, keyword, string or nil)"))))
...
(defn degree->int
  ...
  (if (some #{degree} (keys DEGREE))
    (degree DEGREE)
    (throw (str "Unable to resolve degree: " degree ". Was expecting a roman numeral in the range :i -> :vii or the nil-note symbol :_"))))
...
(defn degrees->pitches
  ...
  (let [root (note root)]
    (when (nil? root)
      (throw (str "root resolved to a nil value. degrees->pitches requires a non-nil root.")))
    (map (fn [degree]
           (cond
...
(defn rand-chord
  ...
  (let [chord (chord root chord-name)
    ...
    (sort (take num-pitches (shuffle notes)))))
...
(defn hz->midi
  ...
  (js/Math.round (+ 69
                 (* 12
                    (/ (js/Math.log (* freq 0.0022727272727))
                       (js/Math.log 2))))))
...
(defn amp->db
  ...
  [amp]
  (* 20 (js/Math.log10 amp)))

; dbamp
(defn db->amp
  "Convert decibels to linear amplitude."
  [db]
  (js/Math.exp (* (/ db 20) (js/Math.log 10))))
...
(defn nth-equal-tempered-freq
  ...
  [base-freq interval]
  (* base-freq (js/Math.pow 2 (/ interval 12))))
